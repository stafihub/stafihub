...
func networkWithProposalObjects(t *testing.T, n int) (*network.Network, []types.Proposal) {
	...
	for i := 0; i < n; i++ {
		state.ProposalList = append(state.ProposalList, types.Proposal{
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.Equal(t, tc.obj, resp.Proposal)
			}
		})
	}
}
...
func TestListProposal(t *testing.T) {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			require.Subset(t, objs, resp.Proposal)
		}
	})
	...
	t.Run("Total", func(t *testing.T) {
		...
		require.Equal(t, objs, resp.Proposal)
	})
}
