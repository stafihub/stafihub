...
func (m *Relayer) GetAddrs() map[string]bool {
	if m != nil {
		return m.Addrs
	}
	return nil
...
func init() {
	...
	proto.RegisterMapType((map[string]bool)(nil), "stafiprotocol.stafihub.relayers.Relayer.AddrsEntry")
...
var fileDescriptor_40adf04c18425525 = []byte{
	...
	,
}
...
func (m *Relayer) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if len(m.Addrs) > 0 {
		for k := range m.Addrs {
			...
			i -= len(k)
			copy(dAtA[i:], k)
			i = encodeVarintRelayer(dAtA, i, uint64(len(k)))
			i--
			dAtA[i] = 0xa
			i = encodeVarintRelayer(dAtA, i, uint64(baseI-i))
			i--
			dAtA[i] = 0x12
		}
	}
...
func (m *Relayer) Size() (n int) {
	...
	if len(m.Addrs) > 0 {
		for k, v := range m.Addrs {
			_ = k
			_ = v
			mapEntrySize := 1 + len(k) + sovRelayer(uint64(len(k))) + 1 + 1
			n += mapEntrySize + 1 + sovRelayer(uint64(mapEntrySize))
		}
	}
	return n
}
