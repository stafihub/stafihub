...
func TestListEraExchangeRate(t *testing.T) {
	net, objs := networkWithEraExchangeRateObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListEraExchangeRate(), args)
			require.NoError(t, err)
			var resp types.QueryAllEraExchangeRateResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
			require.LessOrEqual(t, len(resp.EraExchangeRate), step)
			require.Subset(t, objs, resp.EraExchangeRate)
		}
	})
	...
	t.Run("Total", func(t *testing.T) {
		...
		require.Equal(t, objs, resp.EraExchangeRate)
	})
}
